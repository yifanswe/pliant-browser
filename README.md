...
```text
User + their chosen AI
          |
          v
Personal browser package
  Declarative UI + local state + behavior plugins
          |
          v
Versioned capability contracts
  Observable state, commands, events, service interfaces
          |
          v
Trusted browser core
  Identity, isolation, permission enforcement,
  lifecycle, resource limits, storage and migrations
          |
          v
Existing web engine + operating-system facilities
```
